...
use sc_client_api::{BlockOf, backend::AuxStore, client::BlockBackend };
...
impl<B, I, C, S, Algorithm> PowBlockImport<B, I, C, S, Algorithm> where
	...
	C: ProvideRuntimeApi<B> + Send + Sync + HeaderBackend<B> + BlockBackend<B> + AuxStore + ProvideCache<B> + BlockOf,
...
pub fn start_mine<B: BlockT, C, Algorithm, E, SO, S, CAW>(
	...
	C: HeaderBackend<B> + BlockBackend<B> + AuxStore + ProvideRuntimeApi<B> + 'static,
	...
	CAW: CanAuthorWith<B>,
{
	'outer: loop {
		...
		let timestamp = now.duration_since(std::time::SystemTime::UNIX_EPOCH).unwrap().as_secs() as u32;
		inherent_digest.push(DigestItem::Other(timestamp.encode()));

		let proposal = futures::executor::block_on(proposer.propose(
			inherent_data,
			inherent_digest.clone(),
			build_time.clone(),
			RecordProof::No,
		...
		for digest in inherent_digest.logs {
			header.digest_mut().push(digest)
		}
